...
struct HexGridElement
{
    ...
    HexGridElement() : ballIndex(-1), springsNeededMask(0), springsAddedMask(0) {}
};
...
class HexBuilder // calculates the hexagonal grid arrangement and mesh topology
{
  ...
  public:
    HexBuilder(PhysicsMesh &_mesh, int _dimension, float _spacing, float _mass)
        : map(-_dimension, +_dimension, -_dimension, +_dimension, HexGridElement()), mesh(_mesh),
          spacing(_spacing), mass(_mass), u1(1 + _dimension), u2(-(1 + _dimension)),
          v1(1 + _dimension), v2(-(1 + _dimension))
    {
    }
  ...
  private:
    ...
    PhysicsVector Location(int u, int v) const
    {
        ...
        return PhysicsVector(spacing * (u + v / 2.0), spacing * (sqrt(0.75) * v), 0.0f, 0.0f);
    }
    ...
    void AddBall(int u, int v, uint8_t springsNeededMask)
    {
        HexGridElement &h = map.at(u, v);
        if (h.ballIndex <
            0) // Add a new ball only if one hasn't been added at this location already.
        {
